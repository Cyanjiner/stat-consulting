...
data$City[data$City %in% c("Htfd","Hartford")] <- "Hartford"
data$City[data$City %in% c("Norwich","NORWICH")] <- "Norwich"
data$City[data$City %in% c("E Hartford","E Htfd","East Hartford")] <- "East Hartford"
data$City[data$City %in% c("middletown","Middletown")] <- "Middletown"
data$City[data$City %in% c("N Haven","New haven","New Haven","NEW HAVEN")] <- "New Haven"
data$City[data$City %in% c("New Brit","New Britain")] <- "New Britain"
data$City[data$City %in% c("W Haven","West Haven")] <- "West Haven"
data$City[data$City %in% c("waterbury","Waterbury","Wtby")] <- "Waterbury"
```
...
```{r suicide class risk, echo=FALSE}
...
city_table <- city_data %>% 
  ...
  tidyr::drop_na() %>% 
  mutate(class1_prop = `Class 1`/897,
         class2_prop = `Class 2`/448,
         class3_prop = `Class 3`/469) %>% 
  mutate(`Class 1 (n=897)` = paste0(round(class1_prop*100,2), "%"),
         `Class 2 (n=448)` = paste0(round(class2_prop*100,2), "%"),
         `Class 3 (n=469)` = paste0(round(class3_prop*100,2), "%")) %>% 
  select(`Discharged City`,`Class 1 (n=897)`,`Class 2 (n=448)`,`Class 3 (n=469)`)
head(city_table,10) %>% 
